...
document.getElementById('slide2').addEventListener('click', function(){
    ...
    tl.to('#slide2', .5, { scale: 4, opacity: 1, ease: Power4.easeIn},  "-=1.95");
    tl.to('#slide2', .35, { opacity: 0, ease: Power4.easeIn});
    tl.set(".wrap", {opacity: 0, display:"none"});
    tl.set(".img-wrapper", {opacity: 0, display:"none"});
    tl.set(".quote", {opacity: 1, display:"flex"});
    tl.set("#first", {opacity: 0, delay: 8, display:"none"}, "-=.7");
    // second slide start
    // tl.set("#second", {opacity: 1, delay: 8, display:"flex"}, "-=.1");
});
